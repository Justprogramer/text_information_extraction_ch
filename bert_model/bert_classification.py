...
from bert_model import bert_config
...
class MyPro(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "cl_train.tsv")), 'train')

    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "cl_train.tsv")), 'dev')
    ...
    def _create_examples(self, dicts, set_type):
        ...
        if set_type not in ["train", "test", "dev"]:
            raise ValueError("set_type error: {}, should be one of [\"train\", \"test\", \"dev\"]".format(set_type))
        if "train" == set_type:
            dicts = dicts[:int(len(dicts) * 0.8)]
        elif "dev" == set_type:
            dicts = dicts[int(len(dicts) * 0.8):]
...
def main():
    args = Config(bert_config.DEFAULT_CONFIGS)
